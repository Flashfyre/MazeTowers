...
### Iron Chest
A chest with an iron texture. In Maze Towers, these contain items with a loot tier from B to B+. They can also be smelted into iron ingots **[NOT YET IMPLEMENTED]**.
### Gold Chest
A chest with an gold texture. In Maze Towers, these contain items with a loot tier from A to A+. They can also be smelted into gold ingots **[NOT YET IMPLEMENTED]**.
### Diamond Chest
A chest with an diamond texture. In Maze Towers, these contain items with a loot tier from S to S+. They can also be smelted into diamonds **[NOT YET IMPLEMENTED]**.
...
### Gold Item Scanner
An Item Scanner made of gold that takes an item from a player if it matches a key item. This allows for tollbooth mechanics. Like regular Item Scanners, these can be acquired by trading an Engineer shopkeeper for emeralds.
### Redstone Clock
...
### Explosive Arrow
A special arrow that explodes shortly after hitting a block or instantly after hitting an entity. These may only be fired from an Explosive Bow. **These currently cannot be obtained but will probably be dropped by Explosive Creepers eventually.**
### Explosive Bow
A special bow that can shoot Explosive Arrows. There is a slim, random chance that increases with the Level tier of Maze Towers that an item frame will contain an Explosive Bow. If you manage to find one, be careful retrieving it from the item frame because it'd be pretty disappointing to accidentally lose it in the process.
...
* Tower floor generation needs to be debugged
* Locks need to be fixed to be cheat-proof with Vanilla doors
* Chest loot list needs to be completed
* Special Villager shopkeeper prices should be adjusted from just 1 emerald per any item
