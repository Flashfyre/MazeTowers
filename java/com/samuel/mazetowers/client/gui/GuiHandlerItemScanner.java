...
public class GuiHandlerItemScanner implements IGuiHandler {
    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == MOD_TILE_ENTITY_GUI)
            return new ContainerItemScanner(player.inventory,
            	(TileEntityItemScanner) world.getTileEntity(new BlockPos(x, y, z)), player);
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        if (ID == MOD_TILE_ENTITY_GUI)
            return new GuiItemScanner(player.inventory,
            	((TileEntityItemScanner) world.getTileEntity(new BlockPos(x, y, z))));
