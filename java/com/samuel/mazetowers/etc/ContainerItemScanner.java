...
public class ContainerItemScanner extends Container
{
    ...
    public ContainerItemScanner(IInventory playerInventory, IInventory scannerInventory, EntityPlayer player)
    {
    	keyInv = (IInventory) new InventoryBasic("Key", true, 1);
    	keyInv.setInventorySlotContents(0, ((TileEntityItemScanner) scannerInventory)
    		.getKeyStack());
        ...
        for (int j = 0; j < this.numRows; ++j)
        {
            for (int k = 0; k < 9; ++k)
            {
                this.addSlotToContainer(new Slot(scannerInventory, k + j * 9, 8 + k * 18, 18 + j * 18));
            }
        }
        ...
        this.addSlotToContainer(new KeyStackSlot(keyInv, 0, 184, 18,
