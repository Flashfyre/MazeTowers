...
public abstract class MobSpawnerSpecialBaseLogic extends MobSpawnerBaseLogic {
    ...
    private String getEntityNameToSpawn()
    {
        ...
        {
            if (this.baseMobID != null && this.baseMobID.equals("Minecart"))
            {
                this.mobID = this.baseMobID = "MinecartRideable";
            }

            return this.baseMobID;
        }
        else
        {
	...
	private Entity spawnNewEntity(Entity entityIn, boolean spawn)
    {
        ...
        else if (entityIn instanceof EntityLivingBase && entityIn.worldObj != null && spawn)
        {
            ...
            if (entityIn.worldObj.provider.getDimensionId() == -1 &&
            	entityIn instanceof EntitySkeleton) {
            	EntitySkeleton skeleton = (EntitySkeleton) entityIn;
            	...
            	try {
					...
					entityIn.worldObj.spawnEntityInWorld(skeleton);
				}
            } else if (mobID == "ChargedCreeper") {
            	EntityCreeper creeper = (EntityCreeper) entityIn;
            	creeper.getDataWatcher().updateObject(17, Byte.valueOf((byte)1));
            	entityIn.worldObj.spawnEntityInWorld(creeper);
            } else
            	entityIn.worldObj.spawnEntityInWorld(entityIn);
        }
        ...
        if (this.isActivated())
        {
            ...
            else
            {
                ...
                for (int i = 0; i < this.spawnCount; ++i)
                {
                    ...
                    double d1 = (double)(blockpos.getY() - (shouldSpawnAbove ? 0 : 3));
    ...
    {
		this.baseMobID = nbt.getString("BaseEntityId");
    ...
    public void writeToNBT(NBTTagCompound nbt)
    {
        ...
        {
        	nbt.setString("BaseEntityId", getActualEntityName());
            ...
            if (this.getRandomEntity() != null || this.minecartToSpawn.size() > 0)
            {
                ...
                if (this.minecartToSpawn.size() > 0)
                {
                    for (WeightedRandomMinecart weightedrandomminecart :
