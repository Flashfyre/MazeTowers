	...
	public void update() {
		if (!this.worldObj.isRemote) {
			...
			if (hasTower && tower == null) {
				tower = MazeTowers.mazeTowers.getTowerAtCoords(worldObj, (int) posX >> 4, (int) posZ >> 4);
				if (tower == null)
					hasTower = false;
			}
			List list = worldObj.getEntitiesWithinAABB(EntityPlayer.class,
				AxisAlignedBB.fromBounds(posX - 0.5D, posY - 0.5D, posZ - 0.5D, posX + 0.5D, posY + 0.5D, posZ + 0.5D));
			for (int i = 0; i < list.size(); i++) {
				EntityPlayer player = (EntityPlayer) list.get(i);
				PlayerMazeTower props = PlayerMazeTower.get(player);
				if (props != null) {
					int chunkX = 0, baseY = 0, chunkZ = 0, floors = 0, difficulty = 0;
					...
					if (hasTower) {
						...
						towerName = tower.towerTypeName + " Maze Tower" +
							(isUnderground ? " (Underground)" : "");
					}
					MazeTowers.network.sendTo(new PacketMazeTowersGui(chunkX, baseY, chunkZ,
						floors, difficulty, isUnderground, towerName), (EntityPlayerMP) player);
				}
			}
		}
	}
