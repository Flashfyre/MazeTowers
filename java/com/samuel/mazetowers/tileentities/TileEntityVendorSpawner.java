	...
	public void init(boolean isRespawn) {
		...
		if (!isRespawn) {
    		...
    		ArrayDeque<EntityVillager> nearbyEntities =
    			...
    			EntityVillager.class, AxisAlignedBB.fromBounds(
    			posX + 2.0D, posY + 1.5D, posZ + 2.0D, posX - 1.0D, posY - 1.5D,
    			posZ - 1.0D)));
    		while (!nearbyEntities.isEmpty() && (((vendor = nearbyEntities.pop())
    			instanceof EntitySpecialVillager && specialProfessionId != vendor.getProfession()) ||
    			(specialProfessionId == 0 && vendor instanceof EntitySpecialVillager)))
    			vendor = null;
    		if (tower != null)
		...
		if (spawnVillager) {
			vendor = (specialProfessionId != 0) ? new EntitySpecialVillager(worldObj,
